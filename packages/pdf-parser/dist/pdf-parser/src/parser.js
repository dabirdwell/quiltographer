...
exports.QuiltPatternParser = void 0;
exports.parseQuiltPattern = parseQuiltPattern;
const pdf = require("pdf-parse");
const universal_schema_1 = require("../../core/patterns/universal-schema");
...
class QuiltPatternParser {
     ...
     */
    parseMetadata(rawText, sections) {
        ...
        return {
            ...
            size: sizeMatch ? {
                finished: {
                    width: (0, universal_schema_1.parseImperialMeasurement)(sizeMatch[1]) || 0,
                    height: (0, universal_schema_1.parseImperialMeasurement)(sizeMatch[2]) || 0,
                    unit: 'inches'
                }
            } : undefined,
        ...
        for (const line of lines) {
            ...
            if (fabricMatch) {
                fabrics.push({
                    ...
                    amount: (0, universal_schema_1.parseImperialMeasurement)(fabricMatch[2]) || 0,
        ...
        if (squareMatch) {
            return {
                ...
                pieces: [{
                        ...
                        dimensions: {
                            width: (0, universal_schema_1.parseImperialMeasurement)(squareMatch[2]) || 0,
                            height: (0, universal_schema_1.parseImperialMeasurement)(squareMatch[2]) || 0,
                            unit: 'inches'
                        }
                    }]
            };
        ...
        if (rectMatch) {
            return {
                ...
                pieces: [{
                        ...
                        dimensions: {
                            width: (0, universal_schema_1.parseImperialMeasurement)(rectMatch[2]) || 0,
                            height: (0, universal_schema_1.parseImperialMeasurement)(rectMatch[3]) || 0,
                            unit: 'inches'
                        }
                    }]
            };
        ...
        if (stripMatch) {
            return {
                ...
                pieces: [{
                        ...
                        dimensions: {
                            width: (0, universal_schema_1.parseImperialMeasurement)(stripMatch[2]) || 0,
                            height: 42, // WOF
                            unit: 'inches'
                        }
                    }]
     ...
     */
    parseConstructionSteps(section) {
        if (!section)
            return [];
        const steps = [];
        ...
        if (numberedSteps) {
            numberedSteps.forEach((stepText, index) => {
                ...
                if (stepMatch) {
                    ...
                    steps.push(this.createConstructionStep(stepNumber, instruction, `step-${stepNumber}`));
                }
            });
        }
        else {
            ...
            paragraphs.forEach((para, index) => {
                if (para.length > 20) { // Skip very short paragraphs
                    steps.push(this.createConstructionStep(index + 1, para.trim(), `step-${index + 1}`));
                }
            });
        }
        return steps;
     ...
     */
    createConstructionStep(number, instruction, id) {
        // Expand abbreviations if requested
        const clarified = this.options.expandAbbreviations
            ? (0, universal_schema_1.expandAbbreviation)(instruction)
            : instruction;
     ...
     */
    identifyTechniques(instruction) {
        const techniques = [];
        const lower = instruction.toLowerCase();
        if (lower.includes('chain piec'))
            techniques.push('chain-piecing');
        if (lower.includes('nest') && lower.includes('seam'))
            techniques.push('nesting-seams');
        if (lower.includes('half square triangle') || lower.includes('hst'))
            techniques.push('half-square-triangle');
        if (lower.includes('y-seam'))
            techniques.push('y-seams');
        if (lower.includes('paper piec'))
            techniques.push('paper-piecing');
        if (lower.includes('appliq'))
            techniques.push('applique');
        return techniques;
    }
    /**
     * Generate warnings based on instruction content
     */
    generateWarnings(instruction) {
        const warnings = [];
        const lower = instruction.toLowerCase();
    ...
    normalizeUnit(unit) {
        ...
        if (lower.includes('meter'))
            return 'meters';
        if (lower.includes('fat') || lower === 'fq')
            return 'fat quarters';
    ...
    estimateStepTime(instruction) {
        ...
        const lower = instruction.toLowerCase();
        if (lower.includes('press') && !lower.includes('sew'))
            return 2;
        if (lower.includes('cut'))
            return 10;
        if (lower.includes('sew') && lower.includes('row'))
            return 15;
        if (lower.includes('attach') || lower.includes('join'))
            return 10;
        return 5; // default
    }
    detectCategory(text) {
    ...
    generateTags(data) {
        ...
        if (data.metadata.difficulty <= 2)
            tags.push('beginner-friendly');
        if (data.metadata.difficulty >= 4)
            tags.push('advanced');
        // Add technique tags
        ...
        if (data.metadata.size?.finished) {
            ...
            if (area < 2000)
                tags.push('small-project');
            if (area > 5000)
                tags.push('large-project');
        }
        return tags;
    }
    buildTechniqueLibrary(steps) {
...
exports.QuiltPatternParser = QuiltPatternParser;
// Export main parsing function
async function parseQuiltPattern(pdfBuffer, options) {
